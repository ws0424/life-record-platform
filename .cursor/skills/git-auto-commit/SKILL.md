...
description: Git 自动提交工具。当用户说"提交代码"、"git commit"时使用此技能。根据当前开发阶段和修改内容，自动生成语义化的 commit message，执行 git add 和 commit 操作。支持自动识别提交类型（feat/fix/docs/style/refactor等）。适用于快速提交代码变更到本地仓库。
---

# Git 自动提交工具

根据当前开发内容自动生成语义化 commit message 并提交到本地 Git 仓库。
...
**注意：** 此工具只负责本地提交，不会自动推送到远程仓库。如需推送，请手动执行 `git push`。
...
4. **推送操作**
   - 此工具只负责本地提交
   - 推送到远程需要手动执行 `git push`
   - 团队协作时注意先 pull 再 push
